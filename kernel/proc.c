...
uint64 proc_resize(pagetable_t pagetable, uint64 oldsz, uint64 newsz) 
{
    ...
    if(newsz > oldsz){
      for(a = PGROUNDUP(oldsz); a < newsz; a += PGSIZE){
        ...
        memset(pa, 0, PGSIZE);
        if(vm_page_insert(pagetable, a, (uint64)pa, PTE_R | PTE_U | PTE_W | PTE_X) == -1){
          vm_page_free(pa);
          proc_shrink(pagetable, a, oldsz);
          return 0;
        }
...
proc_vmcopy(pagetable_t old, pagetable_t new, uint64 sz)
{
  ...
  for(i = 0; i < sz; i += PGSIZE){
    ...
    if(vm_page_insert(new, i, (uint64)mem, flags) == -1){
      vm_page_free(mem);
      goto err;
    }
  }
...
proc_pagetable(struct proc *p)
{
    ...
    if((pagetable = vm_create_pagetable()) == 0)
      return 0;
...
proc_loadseg(pagetable_t pagetable, uint64 va, void *bin, uint offset, uint sz)
{
  ...
  for(i = 0; i < sz; i += PGSIZE){
    ...
    if((pte = walk_pgtable(pagetable, va + i, 0)) == 0)
      return -1;
